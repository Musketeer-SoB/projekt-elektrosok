...
void transmit (struct collar_message message)
{
  ...
  send_text("I am doing something");
  ...
  time_t cmd_start=millis();
  int millisDuration = message.duration*1000;

  Serial.println("Sending");
  
  while (millis()-cmd_start < millisDuration)
  {
    Serial.print(millis()-cmd_start + " ");
...
{
  collar_message msg;
  msg.id = transmitter_key.toInt();
  msg.channel = CH1;
  
  if (server.arg("vibrate") != "")
  {
    msg.mode = VIBE;
    msg.power = server.arg("v_str").toInt();
    msg.duration = server.arg("v_dur").toInt();
    //build_and_send("0010","1011",server.arg("v_str"),server.arg("v_dur"));
  }
  else if (server.arg("shock") != "")
  {
    msg.mode = SHOCK;
    msg.power = server.arg("s_str").toInt();
    msg.duration = server.arg("s_dur").toInt();
  ...
  //do_log("Sending channel=" + msg.channel + ", cmd=" + msg.mode + ", key=" + msg.id + ", power=" + msg.power );
  transmit(msg);
}
